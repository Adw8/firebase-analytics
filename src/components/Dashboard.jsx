...
const UserDashboard = () => {
  ...
  const modalTableDataYesterday = docID.map((id) => [id]);
  ...
  const modalTableDataSessions = currSessionData.map((user) => [
    (user.session_start &&
      new Date(
        user.session_start.seconds * 1000 + user.session_start.nanoseconds / 1e6
      ).toLocaleString()) ||
      "",
    (user.session_end &&
      new Date(
        user.session_end.seconds * 1000 + user.session_end.nanoseconds / 1e6
      ).toLocaleString()) ||
      "",
  ]);
    ...
    <div>
      ...
      <div className="max-w-4xl mx-auto my-16 p-4 bg-white shadow-md rounded-md">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl mb-4">
            List of users that logged in yesterday
          </h1>
          <button
        ...
        <ModalComponent
          ...
          onClose={closeYesterdayModal}
          headings={modalHeadingsYesterday}
          tableData={modalTableDataYesterday}
        />
      </div>
      ...
      <div className="max-w-4xl mx-auto my-16 p-4 bg-white shadow-md rounded-md">
        ...
        <ModalComponent
          ...
          headings={modalHeadingsSessions}
          tableData={modalTableDataSessions}
        />
      </div>
    </div>
  );
